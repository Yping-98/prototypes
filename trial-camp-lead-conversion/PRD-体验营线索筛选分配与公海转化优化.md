...
|----------|----------|----------|----------|------|
| 线索领取时间筛选 | 时间范围选择器 | 是 | 选择起止日期后点击查询生效 | 位于公海线索转化页面，交互样式沿用现有日期控件 |
| 支付时间筛选 | 时间范围选择器 | 是 | 选择起止日期后点击查询生效 | 同上，与线索领取时间筛选并列展示 |

**枚举值类**

| 字段名称 | 字段说明 | 字段来源 | 备注 |
|----------|----------|----------|------|
| 线索等级-Z | 高级筛选中线索等级筛选项新增 Z 枚举值 | 线索等级体系 | 多选逻辑、筛选交互沿用现有配置 |

### 7.2 【修改】功能

| 功能/字段名称 | 修改项 | 改前 | 改后 | 备注 |
|---------------|--------|------|------|------|
| 线索分配规则-人员保存 | 保存后的人员顺序 | 按组别先后顺序展示 | 保存时系统自动随机打乱全部已选人员顺序后展示 | 每次保存均重新随机；人员选择交互沿用现有配置 |
| 公海线索转化-日期筛选 | 筛选项结构 | 单一日期筛选 | 拆分为「线索领取时间」「支付时间」两个筛选项 | 查询逻辑见下方说明 |
...
|----------|----------|----------|------------------|
| 公海线索转化-查询按钮 | 新增 | 任一时间筛选项未选择完整起止日期 | 提示"请填写完整的开始和结束日期"，不发起查询 |
| 公海线索转化-统计结果 | 新增 | 所选范围内无符合条件的数据 | 各指标展示为 0，页面正常渲染不报错 |
