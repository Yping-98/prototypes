...
| v1.3   | 2026-09-22 | 列表增加操作栏，支持编辑与删除，删除需二次确认 | AI PRD Writer |

## 二、需求概述

- **背景/现状**：WHEN 运营在体验营 CRM 配置投放侧人群排除时，IF 缺少统一的排除包管理入口，THEN 无法按渠道沉淀可复用的排除账户配置。
- **需求目标**：WHEN 运营进入运营工具，THEN 可通过「人群排除包」列表查询、分页浏览，并按渠道规则新增、编辑、删除排除配置。
- **核心逻辑简述**：按渠道分支录入；抖音/千川仅配置投放账户ID；小红书先选已接入账户名称并带出投放账号ID，子账号ID选填；每新增保存一条记录。列表提供编辑与删除；删除需二次确认后才生效。
...
5. 保存时的重复校验与提示。
6. 列表操作栏：编辑、删除；删除需二次确认。
7. 本次不包含批量导入导出、权限细分与埋点；未接入自动推包的小红书账户不进入本期枚举。
...
```mermaid
flowchart TD
    A[运营点击编辑] --> B[打开编辑弹窗并回填]
    B --> C[按渠道规则修改字段]
    C --> D[点击保存]
    D --> E{校验是否通过}
    E -->|必填缺失或与其他行重复| F[提示错误不关闭]
    E -->|通过| G[覆盖当前行并刷新列表]
    F --> E1[流程结束]
    G --> E2[流程结束]
```
...
> 原型为可交互 HTML 页面，可体验列表筛选、分页、新增/编辑及删除二次确认。
...
| 保存 / 取消 | 按钮 | 不适用 | 新增或编辑校验通过后落库并关闭弹窗刷新列表；取消关闭不保存 | 新增提示「保存成功」；编辑提示「保存成功」 |
| 操作-编辑 | 文字按钮 | 不适用 | 打开编辑弹窗，回填当前行；字段与校验同新增 | 添加人、添加时间不因编辑而变更；判重排除当前行 |
| 操作-删除 | 文字按钮 | 不适用 | 打开删除二次确认弹窗 | 未确认前不删除 |
| 删除确认弹窗 | 弹窗 | 不适用 | 文案：确定删除该条人群排除包？删除后不可恢复。提供取消、确定 | 取消关闭弹窗；确定后删除当前行，提示「删除成功」 |
...
| 千川 | 渠道 + 投放账户ID | 同上 |
| 小红书 | 渠道 + 投放账户ID + 子账号ID | 三者完全相同才拦截，提示「该配置已存在，不可重复添加」；未填子账号ID按空值参与判重；同一投放账户ID下不同子账号ID分别落库为新行；编辑时排除当前行后再判重 |
...
| 编辑弹窗 | 新增 | 命中与其他行重复 | 提示「该配置已存在，不可重复添加」，不关闭弹窗 |
| 编辑弹窗 | 新增 | 校验通过 | 覆盖当前行，提示「保存成功」，添加人与添加时间不变 |
| 删除 | 新增 | 点击删除 | 弹出二次确认，不立即删除 |
| 删除确认 | 新增 | 点击取消 | 关闭确认弹窗，记录保留 |
| 删除确认 | 新增 | 点击确定 | 删除当前行，提示「删除成功」，刷新列表 |
